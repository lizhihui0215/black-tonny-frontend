...
- `_core` 认证页面
- 路由守卫与 access 生成链路
- 集中式 frontend mock owner provider

当前 `apps/retail-admin/src/router/guard.ts` 继续走统一 access bootstrap，但登录 provider 不依赖 backend auth。

## 当前判断

当前判断：

- 当前单店主登录仍是集中式 frontend mock，并且这是当前仓库的正式标准模式
...
- 当前 mock 只允许集中收敛在 auth/user API provider 层
- 不允许回到 guard 或页面组件里临时注入 sample token / sample user
...
1. 继续保持当前集中 mock provider 与 guard/access store 一致。
