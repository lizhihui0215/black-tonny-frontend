  ...
  <el-card shadow="never" class="rounded-[28px] border border-slate-200">
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <span class="text-xs text-slate-400">{{ subtitle || '经营摘要与术语说明' }}</span>
      </div>
    </template>
    ...
    <div class="grid gap-6 lg:grid-cols-2">
      <section>
        ...
        <ul v-if="insights?.length" class="mt-3 space-y-2 text-sm text-slate-600">
          <li
      ...
      <section>
        ...
        <ul v-if="tips?.length" class="mt-3 space-y-2 text-sm text-slate-600">
          ...
          >
            <div class="font-medium text-slate-900">{{ tip.term || '术语' }}</div>
            <p class="mt-1 leading-6">{{ tip.meaning || '暂无解释' }}</p>
            <p class="mt-1 text-xs text-slate-400">建议关注：{{ tip.watch || '暂无' }}</p>
          </li>
        </ul>
        <EmptyStateBlock
          v-else
