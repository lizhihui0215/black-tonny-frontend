...
const weeklyStrategy = computed(() => toSentenceList(props.board?.weekly_strategy));
    ...
    <div class="grid gap-5">
      <div class="grid gap-4 lg:grid-cols-3">
        <section class="rounded-2xl border border-amber-100 bg-amber-50/70 p-4">
          ...
          <ul v-if="todayMustDo.length" class="mt-3 space-y-2 text-sm text-slate-600">
            <li v-for="item in todayMustDo" :key="item">{{ item }}</li>
          </ul>
          <p v-else class="mt-3 text-sm text-slate-400">当前没有返回今天必须做的动作。</p>
        </section>

        <section class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <h3 class="text-sm font-semibold text-slate-950">本周策略</h3>
          <ul v-if="weeklyStrategy.length" class="mt-3 space-y-2 text-sm text-slate-600">
            <li v-for="item in weeklyStrategy" :key="item">{{ item }}</li>
          </ul>
          <p v-else class="mt-3 text-sm text-slate-400">当前没有返回本周策略。</p>
        </section>

        <section class="rounded-2xl border border-rose-100 bg-rose-50/70 p-4">
          <h3 class="text-sm font-semibold text-slate-950">风险提醒</h3>
          <ul v-if="riskAlerts.length" class="mt-3 space-y-2 text-sm text-slate-600">
            <li v-for="item in riskAlerts" :key="item">{{ item }}</li>
          </ul>
          <p v-else class="mt-3 text-sm text-slate-400">当前没有返回风险提醒。</p>
        </section>
      </div>
      ...
      <section class="rounded-2xl border border-slate-200 p-4">
        <div class="flex items-center justify-between">
          ...
          <span class="text-xs text-slate-400">
            可用导出 {{ Object.keys(manifest?.available_exports ?? {}).length }} 个
          </span>
        </div>
