...
const committedRange = ref<DateRangeTuple>(normalizeDateRangeInput(props.initialRange));
...
const compactRangeLabel = computed(() => formatUsDateRange(committedRange.value));
...
const activeQuickRange = computed(() => findMatchingQuickRange(draftRange.value));
const activeCompactOptionId = computed(() =>
  compactPanelMode.value === 'custom'
    ? 'custom'
    : (activeQuickRange.value?.id ?? 'custom'),
);
const compactOptionButtonClass = computed(() => (id: QuickRangeId | 'custom') =>
  ...
  >
    <template #reference>
      <span v-if="isCompact" class="relative inline-flex">
        <CalendarRangeIcon class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-base text-slate-400" />
        <button
          :class="triggerClass"
      ...
      >
        <span class="flex items-center gap-3">
          ...
          <span class="block">
            <span :class="subLabelClass" class="block text-[11px] font-semibold uppercase tracking-[0.24em]">
              {{ displayTitle }}
            </span>
      ...
      >
        ...
        >
          <div class="flex items-start justify-between gap-3 border-b border-slate-100 pb-3">
            ...
            <span class="rounded bg-slate-50 px-2 py-1 text-[11px] font-medium text-slate-500">
              {{ draftRangeDays }}
            </span>
          </div>
      ...
      >
        <div class="flex items-center justify-between gap-3">
          <div>
            ...
            <p class="mt-1 text-sm font-semibold text-slate-900">
              {{ isCompact ? formatUsDateRange(draftRange) : formatDateRangeLabel(draftRange) }}
            </p>
          </div>
          <span
            :class="
        ...
        <div class="mt-3 grid gap-3 sm:grid-cols-[1fr_auto_1fr] sm:items-center">
          <label :class="isCompact ? 'rounded-md border border-slate-200 bg-slate-50 px-3 py-3' : 'rounded-2xl border border-slate-200 bg-white px-3 py-3 shadow-sm'">
            <span class="block text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">
              {{ t('page.common.dateRange.startDate') }}
            </span>
            <input
          ...
          <span class="hidden text-sm font-medium text-slate-400 sm:inline-flex">
...
:global(.black-tonny-date-range-popper) {
  ...
  box-shadow: 0 24px 80px -32px rgb(15 23 42 / 0.25);
