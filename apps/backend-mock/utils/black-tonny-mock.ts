...
> = {
  last30days: {
    ...
    outOfSeasonStockQty: createMetric(
      62,
      '件',
      'down',
      8,
    ...
    sizeBreakStyleCount: createMetric(
      11,
      '款',
      'down',
      1,
...
export function createDashboardSummaryFixture(
  ...
  const requestedPreset =
    (query.get('preset') as DashboardSummaryPreset | null) ?? 'last7days';
  const preset = requestedPreset === 'custom' ? 'custom' : requestedPreset;
  const [startDate, endDate] =
    preset === 'custom'
      ? [
          query.get('start_date') ?? '2026-03-10',
          query.get('end_date') ?? '2026-03-16',
        ]
      : resolvePresetRange(preset);
  const compareRange = buildCompareRange(startDate, endDate);
  const effectivePreset =
    preset === 'custom' ? inferPresetFromRange(startDate, endDate) : preset;
