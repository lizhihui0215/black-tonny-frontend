...
describe('frontend AI entry docs', () => {
  it('keeps the low-token collaboration entrypoints aligned', () => {
    ...
    const aiCollaborationGuide = readRepoFile('docs/tooling/ai-collaboration-guide.md');

    expect(agents).toContain('## Task Routing And Minimum Read Set');
    expect(agents).toContain('## Low-Token Collaboration Defaults');
    expect(toolingReadme).toContain('[AI 协作低 Token 操作手册](./ai-token-playbook.md)');
    expect(aiCollaborationGuide).toContain('它不展开低 Token 协作的具体操作细则');
    expect(aiCollaborationGuide).toContain('[AI 协作低 Token 操作手册](./ai-token-playbook.md)');
  });

  it('keeps the current auth and mock-login baseline documented', () => {
    const architecture = readRepoFile('ARCHITECTURE.md');
    const loginEvolution = readRepoFile('docs/maintainers/login-evolution-handbook.md');
    ...
    expect(alignmentAudit).toContain('当前业务 `/api/*` 不依赖 frontend bearer token');
  });
});
